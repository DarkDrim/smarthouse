...
public class MainMenuPageSendBoolValueActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		String label = pMap.get("IncomingFalseText").toString();
		if (!TextUtils.isEmpty(label.trim())) {
			mIncomingFalseText = label;
		}

		label = pMap.get("IncomingTrueText").toString();
		if (!TextUtils.isEmpty(label.trim())) {
			mIncomingTrueText = label;
		}
		
		label = pMap.get("OutgoingFalseText").toString();
		if (!TextUtils.isEmpty(label.trim())) {
			mOutgoingFalseText = label;
		}

		label = pMap.get("OutgoingTrueText").toString();
		if (!TextUtils.isEmpty(label.trim())) {
